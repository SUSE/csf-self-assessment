// The FILL persona: one participant and their partial. It owns
// the participant store as well ( — the browser copy IS the work), so
...
export class Fill {
  ...
  // participant whose name may still be blank. A finalized carries
  ...
  // , so the value arrives from the caller. Never a view, so never
  ...
  constructor(restored: ParticipantState | null, view: RestoredView) {
    ...
    // Mirror on every change, so a refresh resumes exactly.
    $effect(() => {
  ...
  // verbatim exception — removing a claim never deletes answers (,
  // ). the index math keeps the active claim valid after the splice.
