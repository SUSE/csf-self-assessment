...
export async function startDesktop(target: DesktopTarget): Promise<void> {
  ...
  const rendererLocation: RendererArtifactLocation = app.isPackaged
...
export function runDesktop(target: DesktopTarget): void {
  registerDesktopScheme();
  configureDesktopIdentity(app, target);
  installApplicationPolicy(app, target);
  app.on('window-all-closed', () => app.quit());

  void startDesktop(target).catch((error: Error) => {
    console.error(error);
    app.exit(1);
  });
}
