...
export function cleanHostSystem(lane) {
...
export function planCleanHostTest(lane) {
...
export async function prepareCleanHostCandidate(input) {
    ...
    for (const renderer of renderers) {
        const directory = join(input.repositoryDirectory, 'apps', renderer.app, 'dist');
        await mkdir(directory, { recursive: true });
        await copyFile(join(input.candidateDirectory, renderer.file), join(directory, renderer.file));
    }
    const system = cleanHostSystem(input.lane);
    for (const asset of packageAssets(manifest.version).filter((entry) => entry.system === system)) {
        const directory = join(input.repositoryDirectory, 'dist', 'desktop', asset.app);
        await mkdir(directory, { recursive: true });
