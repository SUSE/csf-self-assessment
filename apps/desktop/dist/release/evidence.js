...
import { z } from 'zod';
import { PackageEvidenceSchema, ReleaseError, ReleaseManifestSchema, Sha256Schema, packageAssets, releaseAssetNames, } from './contract.js';
import { createDesktopSbom, validateDesktopSbom, } from './sbom.js';
...
async function parseManifest(candidateDirectory) {
    const file = 'release-manifest.json';
    try {
        return ReleaseManifestSchema.parse(JSON.parse(await readFile(join(candidateDirectory, file), 'utf8')));
    }
    catch {
        throw new ReleaseError({ kind: 'invalid-manifest', file });
    }
}
async function verifyFileEvidence(candidateDirectory, evidence) {
...
export async function stagePackageAssets(system, version, buildDirectory, stageDirectory) {
    const assets = packageAssets(version).filter((asset) => asset.system === system);
    for (const asset of assets) {
        try {
    ...
    for (const asset of assets) {
        await copyFile(join(buildDirectory, asset.app, asset.file), join(stageDirectory, asset.file));
    }
...
export async function assembleCandidate(input) {
    ...
    for (const file of expectedInputNames(input.version)) {
        await copyFile(join(input.inputDirectory, file), join(input.outputDirectory, file));
    }
    const dependencies = [
    ...
    const packages = await Promise.all(packageAssets(input.version).map(async (asset) => packageEvidence(asset, await fileEvidence(input.outputDirectory, asset.file))));
    ...
    await writeFile(join(input.outputDirectory, 'release-manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
    const checksumNames = (await readdir(input.outputDirectory)).sort();
    const checksumLines = await Promise.all(checksumNames.map(async (file) => {
        const bytes = await readFile(join(input.outputDirectory, file));
        return `${sha256(bytes)}  ${file}\n`;
    }));
    await writeFile(join(input.outputDirectory, 'SHA256SUMS'), checksumLines.join(''), 'utf8');
...
export async function verifyCandidate(candidateDirectory) {
    ...
    try {
        await validateDesktopSbom(await readFile(join(candidateDirectory, manifest.sbom.file), 'utf8'));
    }
    catch {
    ...
    const checksumNames = initialNames
        ...
        .sort();
    const expectedChecksums = (await Promise.all(checksumNames.map(async (file) => {
        const bytes = await readFile(join(candidateDirectory, file));
        return `${sha256(bytes)}  ${file}\n`;
    }))).join('');
    const actualChecksums = await readFile(join(candidateDirectory, 'SHA256SUMS'), 'utf8');
