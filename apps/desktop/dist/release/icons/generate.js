...
const VARIANTS = ['author', 'assessment'];
async function renderPngs(sourceUrl, outputDirectory, variant) {
...
async function writeIco(outputDirectory, pngs) {
...
async function writeIcns(outputDirectory, pngs) {
...
async function replaceDirectory(stagedDirectory, outputDirectory) {
...
export async function generateIconSet(sourceUrl, outputDirectoryUrl, variant) {
    const outputDirectory = fileURLToPath(outputDirectoryUrl);
    const parent = dirname(outputDirectory);
    await mkdir(parent, { recursive: true });
    const temporaryDirectory = await mkdtemp(join(parent, `.${basename(outputDirectory)}-generate-`));
    try {
...
async function assertMatchingDirectory(expectedDirectoryUrl, actualDirectoryUrl) {
...
export async function checkCommittedIcons(sourceUrl, committedDirectoryUrl) {
    ...
    try {
        for (const variant of VARIANTS) {
            ...
            await assertMatchingDirectory(generatedUrl, new URL(`${variant}/`, committedDirectoryUrl));
        }
    }
    finally {
...
async function runCli(mode) {
    ...
    for (const variant of VARIANTS) {
        await generateIconSet(sourceUrl, new URL(`${variant}/`, committedUrl), variant);
    }
}
