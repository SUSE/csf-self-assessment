...
export const DesktopJsonFileNameSchema = z
    ...
    .refine((name) => new TextEncoder().encode(name).byteLength <= 255, 'JSON filename exceeds 255 bytes');
export const DesktopJsonTextSchema = z
    .string()
    .refine((text) => new TextEncoder().encode(text).byteLength <= MAX_JSON_FILE_BYTES, 'JSON text exceeds 8 MiB');
