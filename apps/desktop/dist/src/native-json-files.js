...
import { z } from 'zod';
import { DesktopJsonFileNameSchema, DesktopJsonTextSchema, DesktopSaveJsonInputSchema, MAX_JSON_FILE_BYTES, } from './bridge-contract.js';
const SelectedFilePathsSchema = z.tuple([z.string()]);
const SelectedSavePathSchema = z.string().min(1);
const JSON_FILTER = [{ name: 'JSON', extensions: ['json'] }];
export function createNativeJsonFileService(ownerWindow) {
    return {
        async openJsonFile() {
