...
// The Report as a document (report.md §3.1): the cover and the numbered spine,
 ...
 *  reader's calendar. The pure module never reads the clock (invariant #7). */
...
/** Page one, unlosable by a forwarded copy (§2.2.1, invariant #1). */
...
export type ReportCover = {
   ...
   *  name (§4.3) — one derivation, two presentations. */
   ...
   *  counted here (invariant #2). */
  contributors: number;
  /** The completeness ribbon, verbatim (invariant #1). */
  ribbon: RibbonModel;
 ...
 *  Report adds no numbers (invariant #2) — every variant holds a model some
 ...
 *  which transcribes every unit (ADR-0019). A body list's job is the SELECTION
...
export type ReportDocument = {
  ...
  /** `null` = no vendor section at all (invariant #5). */
   ...
   *  target; aggregates nothing (ADR-0019). */
 ...
 *  cover names the estate and the appendix reads `answers` for prose (ADR-0019).
